...
AirMLP (FedAvg)           & Geographic & 46.6\% & $<$10 \\
ProphetWrapper (FedGAM)   & IID        & 57.7\% & 1 \\
ProphetWrapper (FedGAM)   & Geographic & 56.1\% & 1 \\
\bottomrule
\end{tabular}
\vspace{0.5em}
...
\textbf{AirMLP + FedAvg.} The 7-day-ahead ICA forecasting task is intrinsically hard for a memoryless MLP: the model receives a single day's measurements at time~$t$ and must predict the ICA class 7~days later, without access to the intervening trajectory. AirMLP reaches 49.7\% accuracy (IID) and 46.6\% (geographic), both substantially above the 33.3\% random baseline (+16.4~pp IID, +13.3~pp geographic). The 3.1~pp gap between IID and geographic partitions confirms that province-level non-IID increases variance---Ponferrada (industrial, coal-fired) vs.~rural Soria exhibit different NO$_2$ distributions---but FedAvg handles this heterogeneity gracefully without divergence. The accuracy ceiling of this MLP baseline is consistent with prior work on short-term air quality forecasting~\cite{lopezblanco2024airquality}: sequential models (LSTM, GRU) with lag features would be expected to reach 60--75\% on this task; the MLP serves as a reproducible, low-resource baseline for the FL system evaluation.

\textbf{ProphetWrapper + FedGAM.} ProphetWrapper~+~FedGAM reaches 57.7\% (IID) and 56.1\% (geographic), outperforming AirMLP by +8.0~pp and +9.5~pp respectively. Prophet's advantage stems from its explicit annual and weekly seasonality components: NO$_2$ concentrations in Castilla y Le\'on follow strong intra-annual cycles (winter heating, summer traffic patterns) that a time-aware GAM captures natively, while a memoryless MLP must infer them indirectly from single-day snapshots. The IID--geographic gap for Prophet (1.6~pp) is narrower than for AirMLP (3.1~pp), because seasonality is a shared structural property across provinces and federates well. Notably, FedGAM converges in round~1: with 8 years of training data, the MAP estimation of Fourier seasonality coefficients is deterministic regardless of warm-start initialization, so subsequent rounds yield identical parameters. This is a property of the Stan MAP optimizer, not a flaw; it confirms that the FL transport correctly serializes and deserializes the Prophet parameter tensors across all 10 clients.
...
We presented FL-Iroh, a NAT-transparent federated learning system that combines Iroh QUIC P2P transport with CoAP/CoRE service discovery for agricultural IoT deployments. Our evaluation demonstrates: (i)~100\% NAT connectivity success across full-cone NAT and CGNAT via relay fallback; (ii)~93.2\% crop recommendation accuracy under IID data converging in 16 rounds; (iii)~graceful degradation to 86.8\% under severe Non-IID heterogeneity ($\alpha{=}0.1$); (iv)~stable accuracy (93.2--93.6\%) across 0--50\% client churn; (v)~O(1) CoAP discovery at 1.9~ms for up to 100 nodes; and (vi)~algorithm-agnostic transport enabling AirMLP~+~FedAvg (49.7\% ICA accuracy, IID) and ProphetWrapper~+~FedGAM (57.7\%, IID, convergence in round~1) over the same Iroh infrastructure with only a server-side aggregator swap. FL-Iroh enables privacy-preserving collaborative learning for precision agriculture and urban air quality monitoring without requiring static IPs, VPNs, or cloud infrastructure.
