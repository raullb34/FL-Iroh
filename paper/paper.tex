...
% -----------------------------------------------------------------------
\begin{abstract}
Federated Learning (FL) for agricultural IoT faces a fundamental connectivity barrier: smart farming devices deployed behind Network Address Translators (NATs) and Carrier-Grade NAT (CGNAT) cannot reach each other or a central parameter server without costly static IP allocations or VPN infrastructure. We present \textbf{FL-Iroh}, an open-source system that couples the \emph{Iroh} QUIC-based P2P transport with a CoAP/CoRE service discovery layer to enable connectivity-guaranteed FL across NAT-constrained edge devices. Iroh's relay-fallback mechanism ensures 100\% connection success regardless of NAT type, while CoAP provides a lightweight control plane for model-parameter routing under heterogeneous network topologies. We evaluate FL-Iroh on the Crop Recommendation dataset (7 soil/climate features, 22 classes) using a purpose-designed AgriMLP model ($\sim$15K parameters), representative of edge inference on resource-constrained agricultural sensors. Across five experiments: (E1) transport throughput characterization---iroh relay achieves 27.7~Mbps loopback and 2.8~Mbps over a real WAN relay link; (E2) FL convergence under Non-IID data---93.2\% accuracy IID, degrading to 86.8\% at Dirichlet $\alpha{=}0.1$, converging in 16 rounds; (E3) NAT traversal---100\% connectivity success across both full-cone NAT and CGNAT scenarios ($N{=}30$ each); (E5) churn resilience---accuracy stable at 93.2--93.6\% across 0--50\% client dropout rates; and (E6) CoAP discovery overhead---constant 1.9~ms with keyword filter versus O(n) 2.8~ms/node with semantic filtering. FL-Iroh demonstrates that robust, infrastructure-free FL is achievable for precision agriculture at the edge.
\end{abstract}

\begin{IEEEkeywords}
Federated Learning, IoT, Agriculture, NAT Traversal, QUIC, Iroh, CoAP, P2P, Edge Computing, Non-IID
\end{IEEEkeywords}
...
\subsection{AgriMLP Model}

For the Crop Recommendation task, we design AgriMLP: a four-layer fully connected network with architecture $7 \to 64 \to 64 \to 32 \to 22$ (input features $\to$ output classes). Each hidden layer uses Batch Normalization and Dropout (rate 0.2) for regularization. Total parameters: $\sim$15K (60~KB at float32), making it feasible for on-device inference on Cortex-M7/A53 class hardware. Input features are z-score normalized using global statistics computed during a preprocessing step: soil macro-nutrients (N, P, K), temperature, humidity, pH, and rainfall.
...
All experiments run on a single-node HPC cluster (Intel Xeon, 16 cores, no GPU) for E2, E5, E6, and on real hardware (PC with WSL2 + Raspberry Pi~4B, 300~km WAN separation) for E1 distributed and E3. FL experiments use $K{=}5$ clients with FedAvg, $E{=}5$ local epochs per round, and $R{=}100$ global rounds unless stated otherwise. The Crop Recommendation dataset contains 2,200 records across 22 crop classes (100 records/class), partitioned across clients using Dirichlet allocation with concentration $\alpha \in \{0.1, 0.5, 1.0\}$ and IID as a baseline. Code and data are available at \url{https://github.com/raullb34/FL-Iroh}.
...
Each scenario runs $N{=}30$ connection attempts. Results are shown in Table~\ref{tab:e3}.

\begin{table}[t]
\centering
\caption{E3: NAT Traversal Results ($N{=}30$ per scenario)}
\label{tab:e3}
\footnotesize
\begin{tabular}{l r r r r}
\toprule
...
\midrule
net\_nat1  (full-cone) & 0.0 & 100.0 & 0.0 & 40--55 \\
net\_cgnat (CGNAT)     & 0.0 & 100.0 & 0.0 & 50--430 \\
\bottomrule
\end{tabular}
\end{table}

\textbf{The key result is 0\% failure rate in both scenarios.} All connections are established via relay, confirming Iroh's DERP fallback as a universal connectivity mechanism. While hole-punching did not succeed in either scenario---consistent with the geographic separation and ISP CGNAT policies observed---the relay provides seamless connectivity with latencies of 40--430~ms, acceptable for FL model exchange.
...
We evaluate FedAvg convergence on the Crop Recommendation dataset with AgriMLP under four partition strategies. All runs use $K{=}5$ clients, $E{=}5$ local epochs, cross-entropy loss, Adam optimizer (lr=0.001), $R{=}100$ rounds. Results in Table~\ref{tab:e2}.

\begin{table}[t]
\centering
\caption{E2: FL Convergence on Crop Recommendation (AgriMLP, $R{=}100$)}
\label{tab:e2}
\footnotesize
\begin{tabular}{l r r r r}
\toprule
...
Accuracy remains stable at 93.2--93.6\% across all churn rates, with no meaningful degradation even at 50\% dropout. All 100 training rounds complete in every configuration. This robustness arises from two factors: (i) FL-Iroh uses Iroh's connection re-establishment mechanism to reconnect dropped clients automatically in subsequent rounds, and (ii) FedAvg aggregation is robust to partial participation when data is approximately IID.

The result validates FL-Iroh's suitability for real agricultural deployments where device availability cannot be guaranteed. Iroh's relay-fallback ensures reconnecting clients can always reach the server, even if their IP address or NAT mapping has changed during the dropout period.
...
FL-Iroh is implemented in Python with Iroh bindings and aiocoap, deployable on any Python 3.11+ environment including Raspberry Pi OS, NVIDIA Jetson, and containerized edge nodes. The Iroh node ID (public key) serves as a persistent device identifier across IP changes, making the system resilient to DHCP lease renewals and cellular handoffs---particularly important for mobile agricultural sensors on tractors or drones.

% -----------------------------------------------------------------------
\section{Conclusion}
\label{sec:conclusion}

We presented FL-Iroh, a NAT-transparent federated learning system that combines Iroh QUIC P2P transport with CoAP/CoRE service discovery for agricultural IoT deployments. Our evaluation demonstrates: (i) 100\% NAT connectivity success across full-cone NAT and CGNAT via relay fallback; (ii) 93.2\% crop recommendation accuracy under IID data converging in 16 rounds; (iii) graceful degradation to 86.8\% under severe Non-IID heterogeneity ($\alpha{=}0.1$); (iv) stable accuracy (93.2--93.6\%) across 0--50\% client churn; and (v) O(1) CoAP discovery at 1.9~ms for up to 100 nodes without filtering. FL-Iroh enables privacy-preserving collaborative learning for precision agriculture without requiring static IPs, VPNs, or cloud infrastructure, making it accessible to small and medium farms.

Future work includes: evaluation with larger agronomy datasets and computer-vision crop disease models; integration of FedProx for improved Non-IID robustness; self-hosted DERP relay deployment for higher throughput; and differential privacy integration for normalization statistics protection.
