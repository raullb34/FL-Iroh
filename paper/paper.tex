...
\midrule
net\_nat1  (full-cone) & 0.0 & 100.0 & 0.0 & 50--90 (cold start: 900) \\
...
\begin{table}[t]
\centering
\caption{E6: 7-Day-Ahead ICA Forecasting Results ($K{=}10$, $R{=}50$, test year~2019)}
\label{tab:e6}
\footnotesize
...
E6 crystallises the central design claim of FL-Iroh: the transport layer does not constrain the aggregation algorithm. Because the Iroh/QUIC data plane transmits any \texttt{torch.state\_dict()} as an opaque binary blob, operators are free to choose the aggregation semantics that best fit their model family. For GAM-family models such as Prophet, applying FedAvg would average trend parameters that encode incompatible local phenomena (Ponferrada coal plant vs.~rural Soria), corrupting every client's model. FedGAM instead applies \emph{partial aggregation}: only the Fourier seasonality coefficients---which represent climatological cycles shared across all Castilla y Le\'on provinces---are federated, while trend parameters remain local. In our current results, the federated Prophet GAM (FedGAM) attains 57.1\% accuracy against 49.7\% for the neural AirMLP+FedAvg baseline, a 7.4~pp difference. This comparison, however, varies \emph{both} the model family and the aggregation rule, and therefore does not by itself isolate the benefit of partial aggregation. Attributing the gain to aggregation alone requires holding the model fixed---a Prophet+FedAvg ablation against Prophet+FedGAM on identical data and transport; our implementation supports this directly through a single server-side change (\texttt{aggregator\_fn}), and we treat the present model-vs-aggregation confound explicitly as a limitation (\S\ref{sec:limitations}). The transport's role here is solely to make either aggregation rule deployable without protocol changes.
...
\textbf{Model-vs-aggregation confound in E6.} The headline E6 figures compare Prophet+FedGAM (57.1\%) against the neural AirMLP+FedAvg baseline (49.7\%), varying both the model family and the aggregation rule simultaneously. Isolating the effect of partial aggregation requires a same-model Prophet+FedAvg baseline on identical data and transport; our implementation supports this ablation through a single server-side change (\texttt{aggregator\_fn=fedavg\_aggregate}), and we report it once the GAM-federation runs are finalized. The numbers above should therefore be read as a model-plus-aggregation comparison, not as an aggregation-only result.
