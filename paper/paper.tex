...
ProphetWrapper (FedGAM)   & Geographic & 56.4\% & 1 \\
\bottomrule
\end{tabular}
\vspace{0.5em}
...
We presented FL-Iroh, a NAT-transparent federated learning system that combines Iroh QUIC P2P transport with CoAP/CoRE service discovery for agricultural IoT deployments. Our evaluation demonstrates: (i)~100\% NAT connectivity success across full-cone NAT and CGNAT via relay fallback; (ii)~93.2\% crop recommendation accuracy under IID data converging in 16 rounds; (iii)~graceful degradation to 86.8\% under severe Non-IID heterogeneity ($\alpha{=}0.1$); (iv)~stable accuracy (93.2--93.6\%) across 0--50\% client churn; (v)~O(1) CoAP discovery at 1.9~ms for up to 100 nodes; and (vi)~algorithm-agnostic transport enabling AirMLP~+~FedAvg (49.7\% ICA accuracy, IID) and ProphetWrapper~+~FedGAM (57.1\%, IID, convergence in round~1) over the same Iroh infrastructure with only a server-side aggregator swap. FL-Iroh enables privacy-preserving collaborative learning for precision agriculture and urban air quality monitoring without requiring static IPs, VPNs, or cloud infrastructure.
