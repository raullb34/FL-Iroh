...
\begin{highlights}
\item Direct QUIC hole-punching succeeds on 98\% of WAN connections across NAT, CGNAT and firewall scenarios, with an end-to-end-encrypted relay as a guaranteed fallback.
\item CoAP provides an O(1), 1.9~ms control plane for round coordination and client discovery; model tensors flow separately over Iroh/QUIC.
\item Algorithm-agnostic transport federates both neural networks (AirMLP) and statistical GAMs (Prophet/FedGAM) with a single server-side change.
\item Validated on agricultural (Crop Recommendation) and environmental (Castilla y Le\'on air quality) IoT domains on real PC$\leftrightarrow$Raspberry~Pi hardware across a 300~km WAN.
\end{highlights}

\begin{keywords}
Federated Learning \sep IoT \sep Agriculture \sep Air Quality \sep NAT Traversal \sep QUIC \sep Iroh \sep CoAP
\end{keywords}
