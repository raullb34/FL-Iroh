...
\begin{enumerate}
    ...
    \item \textbf{CoAP/CoRE}~\cite{rfc7252} como plano de control ligero para el descubrimiento de servicios FL (directorio de recursos, coordinación de rondas), usando serialización CBOR para mínima sobrecarga en enlaces con restricciones.
...
\begin{itemize}
    ...
    \item \texttt{/fl/round}: Un PUT del servidor desencadena una nueva ronda de entrenamiento, portando metadatos ligeros de ronda (\texttt{round\_id}, \texttt{model\_hash}, ID de nodo del servidor); los clientes descargan los tensores del modelo global por Iroh/QUIC (ALPN \texttt{fl-model/1}).
...
\paragraph{Paridad de precisi\'{o}n.}
Para confirmar que la elecci\'{o}n de transporte no afecta la calidad del modelo, ejecutamos la misma carga FedAvg (SimpleCNN sobre CIFAR-10, $K{=}10$ clientes, $R{=}100$ rondas, $n{=}5$ semillas, usando las mismas semillas de replicaci\'{o}n que E2) a trav\'{e}s del harness compatible con Flower de E8 y comparamos la precisi\'{o}n final de test frente a las mediciones E2 de FL-Iroh (Tabla~\ref{tab:e7acc}). Bajo particiones IID y no-IID severa ($\alpha{=}0,1$) ambos transportes son estad\'{i}sticamente indistinguibles ---cada media cae dentro del IC~95\% del otro--- confirmando que el plano de datos P2P sin servidor de FL-Iroh no impone penalizaci\'{o}n de precisi\'{o}n frente a un framework convencional en estrella. Esta paridad es esperable por construcci\'{o}n (ambos ejecutan FedAvg con hiperpar\'{a}metros y semillas id\'{e}nticos) y a\'{i}sla la contribuci\'{o}n de FL-Iroh a la capa de transporte.

\begin{table}[t]
\centering
\caption{Paridad de precisi\'{o}n agn\'{o}stica al transporte (CIFAR-10, SimpleCNN, $K{=}10$, $R{=}100$, $n{=}5$ semillas; precisi\'{o}n final de test, media con IC~95\%)}
\label{tab:e7acc}
\footnotesize
\renewcommand{\arraystretch}{1.25}
\begin{tabular}{l c c}
\toprule
\textbf{Partici\'{o}n} & \textbf{FL-Iroh (E2)} & \textbf{Harness Flower (E8)} \\
\midrule
...
La caída de precisión de 21,7 puntos porcentuales de IID a $\alpha{=}0,1$ (65,9\%~$\to$~44,2\%) es esperada con FedAvg bajo heterogeneidad severa en CIFAR-10. Esta degradación cuantifica el costo del sesgo de etiquetas en FL, consistente con la literatura previa~\cite{li2020federated,scaffold2020}. Sin embargo, reemplazar FedAvg por FedProx~\cite{fedprox2020} o SCAFFOLD~\cite{scaffold2020} podría recuperar 5--10 puntos porcentuales con mayor heterogeneidad, ya que estos algoritmos regularizan explícitamente contra la deriva de gradientes. La capa de transporte de FL-Iroh es agn\'{o}stica al algoritmo; cambiar la estrategia de agregaci\'{o}n no requiere cambios en los componentes de Iroh o CoAP.
