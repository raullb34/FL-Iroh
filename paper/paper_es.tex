...
\begin{table*}[t]
\centering
\caption{E3: Resultados de Traversal NAT ($N{=}30$ transferencias por escenario)}
\label{tab:e3}
\footnotesize
\begin{tabular}{l r r r r}
\toprule
\textbf{Escenario} & \textbf{Transf.} & \textbf{\'{E}xito (\%)} & \textbf{Fallido (\%)} & \textbf{Tipo conex.} \\
\midrule
net\_lan   (l\'{i}nea base LAN) & 30 & 100,0 & 0,0 & sin clasificar \\
net\_nat1  (cono completo)     & 30 & 100,0 & 0,0 & sin clasificar \\
net\_nat2  (sim\'{e}trico)      & 30 & 100,0 & 0,0 & sin clasificar \\
net\_cgnat (CGNAT)             & 30 & 100,0 & 0,0 & sin clasificar \\
net\_fw443 (cortafuegos :443)  & 30 & 100,0 & 0,0 & sin clasificar \\
\bottomrule
\end{tabular}
\end{table*}

\textbf{El resultado clave es una tasa de fallo del 0\% en los cinco escenarios.} Todas las transferencias se completan con éxito ---incluso bajo CGNAT y un cortafuegos limitado al puerto~443---, confirmando que la capa de conexión de Iroh proporciona éxito de establecimiento universal en las configuraciones NAT y de cortafuegos evaluadas. Señalamos una limitación de medición: en nuestro arn\'{e}s la API de tipo de conexión de Iroh devolvió \emph{sin clasificar} (\texttt{unknown}) para el 100\% de las conexiones, por lo que no podemos reportar un desglose cuantitativo directo-vs-relay (\S\ref{sec:limitations}). Dada la separación geográfica de 300~km, los ISPs distintos y las condiciones CGNAT/cortafuegos, el retroceso a relay DERP es el transporte esperado en los escenarios WAN, pero reportamos únicamente lo que nuestra instrumentación midió directamente: 100\% de éxito de establecimiento. La latencia de transferencia en estado estacionario es de 50--93~ms; el establecimiento inicial de conexión (arranque en frío) tarda $\sim$900~ms para net\_nat1 y $\sim$400~ms para net\_cgnat, con eventuales reconexiones de $\sim$1060~ms cuando la conexión QUIC agota su tiempo de espera entre mediciones. Estas latencias son aceptables para el intercambio de modelos FL donde el entrenamiento local domina el tiempo de ronda.

La evaluación actual representa el peor caso conservador para los escenarios WAN: dos pares sin infraestructura de enrutamiento compartida, donde el CGNAT y las restricciones de puerto tensionan la capa de conectividad. Incluso bajo estas condiciones, FL-Iroh logra conectividad total.
...
FL-Iroh logra un 100\% de éxito en establecimiento de conexión en las cinco configuraciones NAT y de cortafuegos probadas (E3, 0\% de fallo). Nuestro arn\'{e}s no clasific\'{o} el tipo de conexión (directo vs.~relay), por lo que el rendimiento se reporta para los dos caminos que pudimos medir directamente: relay iroh en bucle local (27,2~Mbps, preliminar) y relay iroh WAN (2,8~Mbps). La arquitectura relay-primero intercambia rendimiento en el camino directo por conectividad universal. Para actualizaciones del tamaño de AgriMLP ($\sim$60~KB), un push de modelo único tarda $\approx$200~ms al rendimiento del relay WAN ---despreciable comparado con el tiempo de entrenamiento local ($\sim$2--5~s por ronda en una Raspberry Pi). Para modelos más grandes (p. ej., ResNet-18 con 100~MB), el rendimiento del relay se convierte en el cuello de botella: una transferencia de 100~MB tarda $\approx$285~s a 2,8~Mbps. FL-Iroh es así más adecuado para modelos de borde ligeros; el FL de modelos grandes en relays WAN se beneficiaría de compresión de gradientes~\cite{konecny2016} o un servidor relay auto-hospedado.

La ausencia de un desglose medido directo-vs-relay en nuestras pruebas WAN es una limitación del arn\'{e}s (\S\ref{sec:limitaciones}); la separación geográfica observada (300~km, diferentes ISPs) y la configuración CGNAT hacen del retroceso a relay el transporte esperado. Se esperaría que el hole-punching directo tuviese éxito para dispositivos del mismo ISP o dentro de la misma red de campus. Nuestros resultados representan una línea base conservadora; en despliegues IoT agrícolas gestionados (p. ej., granjas usando un único proveedor LTE), la conectividad directa podría ser mayor.
...
\textbf{Conectividad: establecimiento medido, tipo de camino sin clasificar.} E3 demuestra un 100\% de éxito en \emph{establecimiento de conexión} (0\% de fallo) en cinco escenarios NAT/cortafuegos, pero la API de tipo de conexión de Iroh en nuestro arn\'{e}s devolvió \texttt{unknown} para el 100\% de las conexiones, por lo que no podemos reportar un desglose cuantitativo hole-punch-directo-vs-relay. Bajo las condiciones WAN evaluadas (300~km, ISPs distintos, CGNAT, cortafuegos puerto~443), el retroceso a relay DERP es el camino esperado, pero reportamos únicamente el éxito de establecimiento que medimos directamente. Instrumentar el tipo de conexión (p.\,ej., vía \texttt{remote\_info()} de Iroh cuando sea estable en nuestro binding) es trabajo futuro.
